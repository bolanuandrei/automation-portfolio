...
Weekly is too slow. A layout break on Monday sits live until the following
Saturday. I'd move to daily, and add a lightweight CSS-payload check as a
cheap pre-filter so the vision model only runs when something actually
changed — cutting cost while increasing frequency. I'd also persist results
so I could show a trend rather than a single pass/fail per run.
